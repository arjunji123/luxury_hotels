...
function TravelNewsPage() {
  ...
  // Calculate total pages dynamically
  const totalPages = travelNews?.content
    ? Math.ceil(travelNews.content.length / perPage)
    : 1;

  // Extract hotels for the current page
  const paginatedHotels =
    travelNews?.content?.slice(
  ...
  return (
    <>
      <div
        className="  travel-container whater-effect section-padding "
        data-aos="zoom-out-up"
      >
        <div className="row ">
          {paginatedHotels?.map((hotel, index) => (
          <div className="col-lg-3 col-md-4 col-sm-6 mt-4" key={index}>
          <div className="hotel-cards travel-hotel-cards">
            {/* Image Section */}
            <div className="parent-image m-0">
              <div className="hotel-img mt-0 rounded-none overflow-hidden">
                <img
                  className="hotel-inner-img rounded-lg"
            ...
            {/* Content Section */}
            <div className="hotel-content">
              <h4 className="hotel-name text-center">
                {truncateText(hotel?.news_title, 4)}
              </h4>
              <div
                className="teams-name cursor-pointer"
                onClick={() => handleRoute(hotel?.slug)}
              >
                {truncateText(hotel?.business_name, 4)}
              </div>
              <p className="mb-2 text-black text-[15px] mt-2">
                {hotel?.createdAt ?? "15 May 2020 9:00 am"}
              </p>
              <div className="teams-role">{hotel.country?.country}</div>
              <button
                className="text-[#846316] cursor-pointer"
                onClick={() => handleRoute(hotel?.slug)}
              >
                READ MORE
              </button>
